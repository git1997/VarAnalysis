...
public class ElementManager {
	...
	private ElementManager outerScopeElementManager; // Manages the elements in
														// the outer scope
	 ...
	 */
	public ElementManager(ElementManager outerScopeElementManager,
			String functionName) {
	...
	/**
	 * Returns the elementManager that contains this elementManager and has a
	...
	public PhpVariable getVariableFromFunctionScope(String variableName) {
		...
		if (phpVariable != null)
			...
			return outerScopeElementManager
	...
	public void addGlobalVariable(VariableNode globalVariableNode) {
		...
		this.getFunctionScopeElementManager().globalVariableNames
	...
	public void setPredefinedConstantValue(String constantName,
		...
		this.getProgramScopeElementManager().putVariableInCurrentScope(
				phpConstant);
	}
	...
	public DataNode getPredefinedConstantValue(ScalarNode scalarNode) {
		...
		PhpVariable phpConstant = this.getProgramScopeElementManager()
	...
	public boolean containsFunctionInStack(String functionName) {
		return this.getProgramScopeElementManager().functionStack
				.contains(functionName);
	}

	public ArrayList<String> getFunctionStack() {
		return new ArrayList<String>(
				this.getProgramScopeElementManager().functionStack);
	}
	...
	public boolean containsFileInStack(File fileName) {
		return this.getProgramScopeElementManager().fileStack
				.contains(fileName);
	}

	public ArrayList<File> getFileStack() {
		return new ArrayList<File>(
				this.getProgramScopeElementManager().fileStack);
	}
	...
	public HashSet<File> getInvokedFiles() {
		return new HashSet<File>(
				this.getProgramScopeElementManager().invokedFiles);
	}
	...
	public void updateWithBranches(LiteralNode conditionString,
			ElementManager trueBranchElementManager,
			ElementManager falseBranchElementManager) {
		// If a branch has a return/exit statement, update the variables in the
		// current scope with the other branch
		boolean trueBranchTerminated = (trueBranchElementManager != null && (trueBranchElementManager
				.hasReturnStatement() || trueBranchElementManager
				.hasExitStatement()));
		boolean falseBranchTerminated = (falseBranchElementManager != null && (falseBranchElementManager
				.hasReturnStatement() || falseBranchElementManager
				.hasExitStatement()));

		if (trueBranchTerminated || falseBranchTerminated) {
			if (trueBranchTerminated && !falseBranchTerminated
					&& falseBranchElementManager != null)
				this.updateVariableTable(falseBranchElementManager);
			else if (!trueBranchTerminated && falseBranchTerminated
					&& trueBranchElementManager != null)
		...
		HashSet<String> variableNamesInTrueBranch = (trueBranchElementManager != null ? trueBranchElementManager
				.getRegularVariableNames() : new HashSet<String>());
		HashSet<String> variableNamesInFalseBranch = (falseBranchElementManager != null ? falseBranchElementManager
				.getRegularVariableNames() : new HashSet<String>());
		...
		for (String variableName : variableNamesInEitherBranch) {
			PhpVariable variableInTrueBranch = (trueBranchElementManager != null ? trueBranchElementManager
					.getVariableFromFunctionScope(variableName) : this
					.getVariableFromFunctionScope(variableName));
			PhpVariable variableInFalseBranch = (falseBranchElementManager != null ? falseBranchElementManager
					.getVariableFromFunctionScope(variableName) : this
					.getVariableFromFunctionScope(variableName));

			DataNode dataNodeInTrueBranch = (variableInTrueBranch != null ? variableInTrueBranch
					.getDataNode() : null);
			DataNode dataNodeInFalseBranch = (variableInFalseBranch != null ? variableInFalseBranch
					.getDataNode() : null);
			DataNode compactSelectNode = new SelectNode(conditionString,
					dataNodeInTrueBranch, dataNodeInFalseBranch).compact();
		...
		// in both branches.
		if (trueBranchElementManager != null
				&& trueBranchElementManager.containsSpecialVariableOutput()
				|| falseBranchElementManager != null
				&& falseBranchElementManager.containsSpecialVariableOutput()) {
			PhpVariable variableInTrueBranch = (trueBranchElementManager != null ? trueBranchElementManager
	...
	private void updateVariableTable(ElementManager branchElementManager) {
		...
		for (String variableName : branchElementManager
				.getRegularVariableNames()) {
			PhpVariable phpVariable = branchElementManager
					.getVariableFromCurrentScope(variableName);
			this.putVariableInCurrentScope(phpVariable);
		}
		// Update output
	...
	public void updateWithLoop(LiteralNode conditionString,
			ElementManager loopElementManager) {
		// Update regular variables
		HashSet<String> variableNamesInsideLoop = loopElementManager
				.getRegularVariableNames();
		for (String variableName : variableNamesInsideLoop) {
			PhpVariable variableBeforeLoop = this
					.getVariableFromFunctionScope(variableName);
			PhpVariable variableInsideLoop = loopElementManager
					.getVariableFromCurrentScope(variableName);
	...
	private DataNode getAppendedStringValue(DataNode stringValueBeforeLoop,
		...
		if (stringValuesBeforeLoop.size() + 1 == stringValuesAfterLoop.size())
			...
			for (int i = stringValuesBeforeLoop.size(); i < stringValuesAfterLoop
					.size(); i++)
	...
	public void updateAfterFunctionExecution(
			ElementManager functionElementManager,
		...
		for (FormalParameterNode formalParameterNode : formalParameterNodes) {
			if (formalParameterNode.isReference()) {
				...
				if (!(argumentExpressionNodes.get(parameterIndex) instanceof VariableNode)) {
					MyLogger.log(
							MyLevel.TODO,
							"In ElementManager.updateAfterFunctionExecution: Reference parameter is not of type VariableNode.");
					continue;
				}

				String referencedVariableName = ((VariableNode) argumentExpressionNodes
						.get(parameterIndex)).resolveVariableName(null);
				PhpVariable phpVariable = new PhpVariable(
						referencedVariableName);
				phpVariable.setDataNode(functionElementManager
		...
		for (String globalVariableName : functionElementManager
				.getGlobalVariableNames()) {
			PhpVariable variableInsideFunction = functionElementManager
	...
	public void addCurrentOutputToFinalOutput() {
		...
		PhpVariable finalOutput = this.getProgramScopeElementManager()
		...
		if (finalOutput == null) {
			...
			if (isTrueBranch)
				selectNode = new SelectNode(conditionString,
						currentOutput.getDataNode(), finalOutput.getDataNode());
			else
				selectNode = new SelectNode(conditionString,
						finalOutput.getDataNode(), currentOutput.getDataNode());
	...
	public void addReturnValue(DataNode currentReturnValue) {
		PhpVariable finalReturn = this.getFunctionScopeElementManager()
		...
		if (finalReturn == null) {
			...
			this.getFunctionScopeElementManager().putVariableInCurrentScope(
					finalReturnVariable);
		} else {
			SelectNode selectNode;
			if (isTrueBranch)
				selectNode = new SelectNode(conditionString,
						currentReturnValue, finalReturn.getDataNode());
			else
				selectNode = new SelectNode(conditionString,
						finalReturn.getDataNode(), currentReturnValue);
	...
	public void removeReturnValue() {
		this.getFunctionScopeElementManager().variableTable
	...
	public PhpVariable getFinalOutput() {
		return this.getProgramScopeElementManager()
				.getVariableFromCurrentScope(SPECIAL_VARIABLE_FINAL_OUTPUT);
	}

	public PhpVariable getReturnValue() {
		return this.getFunctionScopeElementManager()
				.getVariableFromCurrentScope(SPECIAL_VARIABLE_RETURN);
	}
