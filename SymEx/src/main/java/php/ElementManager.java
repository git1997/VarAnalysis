...
public class ElementManager {
	...
	private Stack<File> fileStack = new Stack<File>();			// Used in PROGRAM scope only
	private HashSet<File> invokedFiles = new HashSet<File>();	// Used in PROGRAM scope only
	...
	public DataNode getPredefinedConstantValue(ScalarNode scalarNode) {
		...
		/* Handle PHP system constants */
			...
			return new LiteralNode(getWorkingDirectory() + StringUtils.getFileSystemSlash() + peekFileFromStack());
	...
	public ArrayList<File> getFileStack() {
		return new ArrayList<File>(this.getProgramScopeElementManager().fileStack);
	}
	...
	public HashSet<File> getInvokedFiles() {
		return new HashSet<File>(this.getProgramScopeElementManager().invokedFiles);
	}
