...
public class SelectNode extends DataNode {
	...
	private LiteralNode conditionString; // Use a literal node to describe and
											// locate the condition string (can
											// be null)
	...
	public SelectNode(LiteralNode conditionString, DataNode nodeInTrueBranch,
			DataNode nodeInFalseBranch) {
	...
	public DataNode clone() {
		...
		clonedNode.nodeInTrueBranch = (this.nodeInTrueBranch != null ? this.nodeInTrueBranch
				.clone() : null);
		clonedNode.nodeInFalseBranch = (this.nodeInFalseBranch != null ? this.nodeInFalseBranch
				.clone() : null);
		return clonedNode;
	}
	...
	public DataNode compact() {
		...
		if (nodeInTrueBranch instanceof LiteralNode)
			...
			nodesInTrueBranch.addAll(((ConcatNode) nodeInTrueBranch)
					.getChildNodes());
		...
		if (nodeInFalseBranch instanceof LiteralNode)
			...
			nodesInFalseBranch.addAll(((ConcatNode) nodeInFalseBranch)
					.getChildNodes());

		int commonNodesBefore = 0;
		for (int i = 0; i < nodesInTrueBranch.size(); i++) {
			if (i < nodesInFalseBranch.size()
					&& nodesInTrueBranch.get(i) == nodesInFalseBranch.get(i))
				commonNodesBefore++;
			else
				break;
		}
		...
		for (int i = 1; i <= nodesInTrueBranch.size() - commonNodesBefore; i++) {
			if (nodesInFalseBranch.size() - i >= 0
					&& nodesInTrueBranch.get(nodesInTrueBranch.size() - i) == nodesInFalseBranch
							.get(nodesInFalseBranch.size() - i))
				commonNodesAfter++;
			else
				break;
		}
		...
		else if (commonNodesBefore + commonNodesAfter + 1 == nodesInTrueBranch
				.size())
			diffNodesInTrueBranch = nodesInTrueBranch.get(commonNodesBefore);
		else {
			diffNodesInTrueBranch = new ConcatNode();
			for (int i = commonNodesBefore; i < nodesInTrueBranch.size()
					- commonNodesAfter; i++)
				((ConcatNode) diffNodesInTrueBranch)
						.appendChildNode(nodesInTrueBranch.get(i));
		}

		DataNode diffNodesInFalseBranch;
		if (commonNodesBefore + commonNodesAfter == nodesInFalseBranch.size())
			diffNodesInFalseBranch = null;
		else if (commonNodesBefore + commonNodesAfter + 1 == nodesInFalseBranch
				.size())
			diffNodesInFalseBranch = nodesInFalseBranch.get(commonNodesBefore);
		else {
			diffNodesInFalseBranch = new ConcatNode();
			for (int i = commonNodesBefore; i < nodesInFalseBranch.size()
					- commonNodesAfter; i++)
				((ConcatNode) diffNodesInFalseBranch)
						.appendChildNode(nodesInFalseBranch.get(i));
		}

		if (diffNodesInTrueBranch != null || diffNodesInFalseBranch != null) {
			SelectNode middleNode = new SelectNode(conditionString,
					diffNodesInTrueBranch, diffNodesInFalseBranch);
			concatNode.appendChildNode(middleNode);
		}

		// Extract the common nodes on the right hand side
		for (int i = 1; i <= commonNodesAfter; i++)
			concatNode.appendChildNode(nodesInTrueBranch.get(nodesInTrueBranch
					.size() - i));

		// Return the compact DataNode that represents the same value as the
		// SelectNode
	...
	final public String getApproximateStringValue() {
		...
		String trueBranchValue = (nodeInTrueBranch != null ? nodeInTrueBranch
				.getApproximateStringValue() : "");
		String falseBranchValue = (nodeInFalseBranch != null ? nodeInFalseBranch
				.getApproximateStringValue() : "");
		if (nodeInTrueBranch != null
				&& !containsSymbolicValues(trueBranchValue))
			return trueBranchValue;
		else if (nodeInFalseBranch != null
				&& !containsSymbolicValues(falseBranchValue))
	...
	private boolean containsSymbolicValues(String string) {
		return string.matches("(?s).*"
				+ SymbolicNode.getSymbolicValueRegularExpression() + ".*") // (?s)
																			// to
																			// consider
																			// line
																			// terminators
				|| string
						.matches("(?s).*"
								+ SelectNode
										.getSymbolicValueRegularExpression()
								+ ".*")
				|| string
						.matches("(?s).*"
								+ ObjectNode
	...
	public Element printGraphToXmlFormat(Document document,
			HashSet<DataNode> parentNodes) {
		Element element = document.createElement(DataModelConfig.XML_SELECT);
		if (conditionString != null) {
			element.setAttribute(DataModelConfig.XML_STRING_VALUE,
					conditionString.getStringValue());
			element.setAttribute(DataModelConfig.XML_FILE_PATH, conditionString
					.getLocation().getLocationAtOffset(0).getFilePath()
					.getPath());
			element.setAttribute(
					DataModelConfig.XML_POSITION,
					Integer.toString(conditionString.getLocation()
		...
		if (!checkForLoops(parentNodes)) {
			if (nodeInTrueBranch != null) {
				...
				Element elementInTrueBranch = document
						.createElement(DataModelConfig.XML_SELECT_TRUE);
			...
			if (nodeInFalseBranch != null) {
				...
				Element elementInFalseBranch = document
						.createElement(DataModelConfig.XML_SELECT_FALSE);
	...
	public String printGraphToGraphvizFormat(HashSet<DataNode> setOfPrintedNodes) {
		...
		if (nodeInTrueBranch != null) {
			string.append(nodeInTrueBranch
					.printGraphToGraphvizFormat(setOfPrintedNodes));
			string.append(GraphvizFormat.printEdge(this, nodeInTrueBranch,
					"True"));
		}
		if (nodeInFalseBranch != null) {
			string.append(nodeInFalseBranch
					.printGraphToGraphvizFormat(setOfPrintedNodes));
			string.append(GraphvizFormat.printEdge(this, nodeInFalseBranch,
					"False"));
		}
	...
	public void visit(DataModelVisitor dataModelVisitor) {
		...
		nodeInTrueBranch.visit(dataModelVisitor);
		if (nodeInFalseBranch!=null)
			nodeInFalseBranch.visit(dataModelVisitor);
	}
}
